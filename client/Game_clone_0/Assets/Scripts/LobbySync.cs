...
public class LobbySync : MonoBehaviour
{
    ...
    // Diccionari persistent per guardar els noms que el servidor aprova al Lobby
    // i poder-los recuperar a l'escena Game.
    ...
    {
        // Esperar unos frames para que el NetworkManager y su CustomMessagingManager estén inicializados
        float timeout = 2f;
        while ((NetworkManager.Singleton == null || NetworkManager.Singleton.CustomMessagingManager == null) && timeout > 0)
        {
        ...
        {
            Debug.LogWarning("[LobbySync] CustomMessagingManager no disponible tras esperar.");
            yield break;
        }
