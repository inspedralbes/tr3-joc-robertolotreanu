...
public class HUDController : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        // Failsafe: Si venim del Lobby i la UI del MenuManager encara és visible (ParrelSync), l'amaguem.
        var allUIDocs = Object.FindObjectsByType<UIDocument>(FindObjectsSortMode.None);
        foreach (var doc in allUIDocs)
        {
            // Si el document conté elements del menú o el script MenuManager, l'amaguem
            if (doc.gameObject.name.Contains("Menu") || doc.rootVisualElement.Q("LoginPanel") != null)
            {
    ...
    private System.Collections.IEnumerator ReturnToMenuSafe()
    {
        ...
        {
            // Si som el host, intentem esborrar la sala abans de tancar
            if (NetworkManager.Singleton.IsServer) {
                yield return StartCoroutine(EliminarSala());
            }
    ...
    private void ActualizarLiveLeaderboard()
    {
        ...
        {
            if (pm == null || pm.isBot.Value) continue;
            
            // Si tenim LocalPlayer, no el tornem a llistar com a "altre"
            if (PlayerMovement.LocalPlayer != null && pm == PlayerMovement.LocalPlayer) continue;
            
            // Si NO tenim LocalPlayer encara, però aquest pm és el nostre (propietat), l'ignorem aquí perquè ja surt a "Tú"
            if (PlayerMovement.LocalPlayer == null && pm.IsOwner && !pm.isBot.Value) continue;

            string pname = pm.playerName.Value.ToString();
            if (string.IsNullOrEmpty(pname) || pname == "Jugador")
                pname = $"Jugador {pm.OwnerClientId}";

            string status = pm.isAlive.Value ? "VIVO" : "MORTO";
            Color color = pm.isAlive.Value ? new Color(0.4f, 0.8f, 1f) : new Color(1f, 0.3f, 0.3f);

            AgregarLineaLeaderboard($"👤 {pname}", status, color);
        }
